    ...
    {
        metalicMap = POI2D_SAMPLER_PAN(_MetallicMask, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float(0.497);
    }
    ...
    void ApplyMetallics(inout float4 finalColor, in float4 albedo)
    {
            ...
            finalColor.rgb += reflecty_lighty_boy_uwu_var_2 * reflection.rgb * fresnelRelflection(albedo) * float4(0.006970477,1,0,1) * tintMap.rgb * tintMap.a;
        #endif
    }
